...
public abstract class AbstractFacade<T> {
    ...
    public T findCacheByPass(Object id) {
    ...
    public List<T> findAllCacheByPass() {
        javax.persistence.criteria.CriteriaQuery cq = getEntityManager().getCriteriaBuilder().createQuery();
        cq.select(cq.from(entityClass));
        return getEntityManager().createQuery(cq).setHint("javax.persistence.cache.retrieveMode", CacheRetrieveMode.BYPASS).getResultList();
    }
    ...
    public List<T> findByCacheByPass(Object query, Object attrValue) {
        javax.persistence.Query q = getEntityManager().createNamedQuery((String)query);
        List<Object> valueList = (List<Object>) attrValue;
        if(valueList.size() == 1)
            q.setParameter(q.getParameters().iterator().next().getName(), valueList.get(0));
        ...
        return q.setHint("javax.persistence.cache.retrieveMode", CacheRetrieveMode.BYPASS).getResultList();
    }
    ...
    public BigDecimal sumBy(Object query, Object attrValue) {
        javax.persistence.Query q = getEntityManager().createNamedQuery((String)query);
        List<Object> valueList = (List<Object>) attrValue;
        if(valueList.size() == 1)
            q.setParameter(q.getParameters().iterator().next().getName(), valueList.get(0));
